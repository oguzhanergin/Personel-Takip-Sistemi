...
namespace wfPersonelTakipSistemi
{
    public partial class frmIzinIslemleri : Form
    {
        ...
        private void dtpIzinBitis_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                if (dtpIzinBitis.Value > dtpIzinBaslangic.Value)
                {
                    txtGunSayisi.Text = (Convert.ToInt32((dtpIzinBitis.Value.Subtract(dtpIzinBaslangic.Value)).Days) + 1).ToString();
                    btnKaydet.Enabled = true;
                }
                else
                {
